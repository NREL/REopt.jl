...
@testset "Outage with Generator, outage simulator, BAU critical load outputs" begin
    ...
    open("outage_results.json","w") do f
        JSON.print(f, results, 4)
    end
    @test results["Generator"]["size_kw"] ≈ 23.13 atol=0.01
    @test (sum(results["Generator"]["electric_to_load_series_kw"][i] for i in 1:9) + 
           sum(results["Generator"]["electric_to_load_series_kw"][i] for i in 101:8760)) == 0
    @test results["ElectricLoad"]["bau_critical_load_met"] == false
    @test results["ElectricLoad"]["bau_critical_load_met_time_steps"] == 0
    @test results["PV"]["size_kw"] == 100
    @test sum(results["PV"]["electric_to_storage_series_kw"]) == sum([
        0.0,
        0.0,
        0.0,
        0.0,
    ...
    @test simresults["resilience_hours_max"] == 248.0
end
