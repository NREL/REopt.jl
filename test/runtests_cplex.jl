...
elseif "CPLEX" in ARGS
    ...
    @testset verbose=true "REopt test set using CPLEX solver" begin
        ...
        @testset verbose=true "CHP" begin
            @testset "CHP Sizing" begin
                ...
                inputs = REoptInputs(s)
                m = Model(GAMS.Optimizer)
                set_optimizer_attribute(m, GAMS.Solver(), "CPLEX", "CPXPARAM_MIP_Tolerances_MIPGap" => 0.01)
                results = run_reopt(m, inputs)
            ...
            @testset "CHP Supplementary firing and standby" begin
                ...
                set_optimizer_attribute(m1, GAMS.Solver(), "CPLEX")
                s = Scenario(data)
                inputs = REoptInputs(s)
                ...
                set_optimizer_attribute(m2, GAMS.Solver(), "CPLEX")
                s = Scenario(data)
                inputs = REoptInputs(s)
        ...
        @testset "Outages with Wind and supply-to-load no greater than critical load" begin
            ...
            m1 = Model(GAMS.Optimizer)
            m2 = Model(GAMS.Optimizer)
            set_optimizer_attribute(m1, GAMS.Solver(), "CPLEX", "CPXPARAM_MIP_Tolerances_MIPGap" => 0.01)
            set_optimizer_attribute(m2, GAMS.Solver(), "CPLEX", "CPXPARAM_MIP_Tolerances_MIPGap" => 0.01)
        ...
        @testset verbose=true "Rate Structures" begin
            ...
            @testset "Custom URDB with Sub-Hourly" begin
                ...
                with_logger(logger) do
                    ...
                    model = Model(GAMS.Optimizer)
                    set_optimizer_attribute(model, GAMS.Solver(), "CPLEX", "CPXPARAM_MIP_Tolerances_MIPGap" => 0.01)
        ...
        @testset "Hybrid GHX and GHP calculated costs validation" begin
            ...
            m3 = Model(GAMS.Optimizer)
            set_optimizer_attribute(m3, GAMS.Solver(), "CPLEX", "CPXPARAM_MIP_Tolerances_MIPGap" => 0.01)
        ...
        @testset "Custom REopt logger" begin
            ...
            # Using filepath
            n1 = Model(GAMS.Optimizer)
            set_optimizer_attribute(n1, GAMS.Solver(), "CPLEX")
            n2 = Model(GAMS.Optimizer)
            set_optimizer_attribute(n2, GAMS.Solver(), "CPLEX")
            ...
            n = Model(GAMS.Optimizer)
            set_optimizer_attribute(n, GAMS.Solver(), "CPLEX")
