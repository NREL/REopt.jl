    ...
    @testset "Imported Xpress Test Suite" begin
        ...
        @testset "FlexibleHVAC" begin
            ...
            @testset "Single RC Model heating only" begin
                ...
                m2 = Model(optimizer_with_attributes(HiGHS.Optimizer, "output_flag" => false, "log_to_console" => false))
                r = run_reopt([m1,m2], d)
                @test any([(occursin("not supported by the solver", msg[1]) || occursin("REopt scenarios solved either with errors or non-optimal solutions", msg[1])) for msg in r["Messages"]["errors"]])
        ...
        @testset "Outages with Wind and supply-to-load no greater than critical load" begin
            ...
            @test any([(occursin("not supported by the solver", msg[1]) || occursin("REopt scenarios solved either with errors or non-optimal solutions", msg[1])) for msg in results["Messages"]["errors"]])
        ...
        @testset "Emissions and Renewable Energy Percent" begin
            ...
            for i in range(1, stop=3)
                ...
                results = run_reopt([m1, m2], inputs)
                if i <= 2
                    solver_error = any([(occursin("not supported by the solver", msg[1]) || occursin("REopt scenarios solved either with errors or non-optimal solutions", msg[1])) for msg in results["Messages"]["errors"]])
                    @test solver_error
                

                elseif !isnothing(ER_target[i])
