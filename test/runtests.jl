...
elseif "CPLEX" in ARGS
    ...
    @testset "Solar and ElectricStorage with cost constants but zero-out ElectricStorage" begin
        ...
        @testset "ElectricTariff 15-minute Interval Tests" begin
            ...
            # Create a base hourly load profile (Hospital reference building)
            ...
            # Instead of simply repeating hourly values, apply sine wave modulation
            # to create realistic intra-hour load variations
            ...
            input_data["ElectricLoad"] = Dict("loads_kw" => fifteen_min_loads)
            input_data["ElectricTariff"] = Dict(
                "blended_annual_demand_rate" => blended_demand_rate,
                "blended_annual_energy_rate" => blended_energy_rate
            )
            ...
            m_fifteen = Model(optimizer_with_attributes(HiGHS.Optimizer, "mip_rel_gap" => 0.01, "output_flag" => false, "log_to_console" => false))
            results_fifteen = run_reopt(m_fifteen, inputs_fifteen)
            ...
            # Test that sub-hourly peaks can be higher than hourly peaks due to sine wave variation
            # Monthly peaks should reflect the highest 15-minute interval, not just hourly averages
            @test any(monthly_peaks_kw_15min .> maximum(base_hourly_loads) * 0.95)  # At least some peaks are higher
