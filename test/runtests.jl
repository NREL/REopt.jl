...
elseif "CPLEX" in ARGS
    ...
    @testset "Backup Generator Reliability" begin
        ...
        @testset "system char" begin
            # using Printf
            for pv_included in [true, false]
                for pv_can_dispatch_without_storage in [true, false]
                    for wind_included in [true, false]
                        for wind_can_dispatch_without_storage in [true, false]
                            for battery_size_kwh in [100,0]
                                for H2_size_kg in [100, 0]
                                    old = system_char_old(pv_included, pv_can_dispatch_without_storage, 
                                                    wind_included, wind_can_dispatch_without_storage,
                                                    battery_size_kwh,  H2_size_kg)
                                    new = system_char_new(pv_included, pv_can_dispatch_without_storage, 
                                                    wind_included, wind_can_dispatch_without_storage,
                                                    battery_size_kwh,  H2_size_kg)
                                                    print(pv_included)
                                                    print(pv_can_dispatch_without_storage)
                                                    print(wind_included)
                                                    print(wind_can_dispatch_without_storage)
                                                    print(battery_size_kwh)
                                                    print(H2_size_kg)
                                                    # @printf("%b, %b, %b, %b, %b, %b\n", pv_included, pv_can_dispatch_without_storage, wind_included, wind_can_dispatch_without_storage, battery_size_kwh, H2_size_kg)
                                    @test old ≈ new
                                end
                            end
                        end
