...
- `size_ton_per_hour`  # Thermal production capacity size of the ASHP_WH [ton/hr]
...
function add_ashp_wh_results(m::JuMP.AbstractModel, p::REoptInputs, d::Dict; _n="")
    r = Dict{String, Any}()
    r["size_ton_per_hour"] = round(value(m[Symbol("dvSize"*_n)]["ASHP_WH"]) / KWH_PER_MMBTU * 0.012, digits=3)
    ...
    @expression(m, ASHPWHThermalProductionSeries[ts in p.time_steps],
        sum(m[:dvHeatingProduction][t,q,ts] for q in p.heating_loads, t in p.techs.ashp_wh)) # TODO add cooling
