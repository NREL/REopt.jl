...
function proforma_results(p::REoptInputs, d::Dict)
    ...
    if "ElectricStorage" in keys(d) && d["ElectricStorage"]["size_kw"] > 0
        ...
        dc_coupled_inverter_kw = d["ElectricStorage"]["dc_couple_inverter_size_kw"]
        capital_cost = (storage.dc_coupled ? dc_coupled_inverter_kw : total_kw) * storage.installed_cost_per_kw + total_kwh * storage.installed_cost_per_kwh
