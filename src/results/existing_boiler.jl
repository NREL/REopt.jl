...
function add_existing_boiler_results(m::JuMP.AbstractModel, p::REoptInputs, d::Dict; _n="")
    r = Dict{String, Any}()
    max_prod_kw = maximum([sum(p.scenario_probabilities[s] * value(sum(m[Symbol("dvHeatingProduction"*_n)][s,"ExistingBoiler",q,ts] for q in p.heating_loads)) for s in 1:p.n_scenarios) for ts in p.time_steps])
	...
	r["thermal_production_series_mmbtu_per_hour"] = 
        round.([sum(p.scenario_probabilities[s] * value(sum(m[:dvHeatingProduction][s, "ExistingBoiler", q, ts] for q in p.heating_loads)) for s in 1:p.n_scenarios) / KWH_PER_MMBTU for ts in p.time_steps], digits=5)
	...
	if !isempty(p.s.storage.types.hot)
        @expression(m, BoilerToHotTESKW[ts in p.time_steps],
		    sum(p.scenario_probabilities[s] * m[:dvHeatToStorage][s, b,"ExistingBoiler",q,ts] for s in 1:p.n_scenarios, b in p.s.storage.types.hot, q in p.heating_loads)
            )
        @expression(m, BoilerToHotTESByQualityKW[q in p.heating_loads, ts in p.time_steps],
            sum(p.scenario_probabilities[s] * m[:dvHeatToStorage][s, b,"ExistingBoiler",q,ts] for s in 1:p.n_scenarios, b in p.s.storage.types.hot)
            )
    else
    ...
    if !isempty(p.techs.steam_turbine) && p.s.existing_boiler.can_supply_steam_turbine
        @expression(m, BoilerToSteamTurbineKW[ts in p.time_steps], sum(p.scenario_probabilities[s] * m[:dvThermalToSteamTurbine][s, "ExistingBoiler",q,ts] for s in 1:p.n_scenarios, q in p.heating_loads))
        @expression(m, BoilerToSteamTurbineByQualityKW[q in p.heating_loads, ts in p.time_steps], sum(p.scenario_probabilities[s] * m[:dvThermalToSteamTurbine][s, "ExistingBoiler",q,ts] for s in 1:p.n_scenarios))
    else
	...
	BoilerToLoadKW = @expression(m, [ts in p.time_steps],
		sum(p.scenario_probabilities[s] * value(m[:dvHeatingProduction][s, "ExistingBoiler",q,ts]) for s in 1:p.n_scenarios, q in p.heating_loads) - BoilerToHotTESKW[ts] - BoilerToSteamTurbineKW[ts]
    )
    ...
    if "DomesticHotWater" in p.heating_loads && p.s.existing_boiler.can_serve_dhw
        @expression(m, BoilerToDHWKW[ts in p.time_steps], 
            sum(p.scenario_probabilities[s] * m[:dvHeatingProduction][s, "ExistingBoiler","DomesticHotWater",ts] for s in 1:p.n_scenarios) - BoilerToHotTESByQualityKW["DomesticHotWater",ts] - BoilerToSteamTurbineByQualityKW["DomesticHotWater",ts]
        )
    else
    ...
    if "SpaceHeating" in p.heating_loads && p.s.existing_boiler.can_serve_space_heating
        @expression(m, BoilerToSpaceHeatingKW[ts in p.time_steps], 
            sum(p.scenario_probabilities[s] * m[:dvHeatingProduction][s, "ExistingBoiler","SpaceHeating",ts] for s in 1:p.n_scenarios) - BoilerToHotTESByQualityKW["SpaceHeating",ts] - BoilerToSteamTurbineByQualityKW["SpaceHeating",ts]
        )
    else
    ...
    if "ProcessHeat" in p.heating_loads && p.s.existing_boiler.can_serve_process_heat
        @expression(m, BoilerToProcessHeatKW[ts in p.time_steps], 
            sum(p.scenario_probabilities[s] * m[:dvHeatingProduction][s, "ExistingBoiler","ProcessHeat",ts] for s in 1:p.n_scenarios) - BoilerToHotTESByQualityKW["ProcessHeat",ts] - BoilerToSteamTurbineByQualityKW["ProcessHeat",ts]
        )
    else
