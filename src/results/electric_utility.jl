...
function add_electric_utility_results(m::JuMP.AbstractModel, p::AbstractInputs, d::Dict; _n="")
    ...
    if :WHL in p.s.electric_tariff.export_bins
        if sum(value.(m[Symbol("WHL_benefit"*_n)])) - 1e8-1000*sum([ld*rate for (ld,rate) in zip(p.s.electric_load.loads_kw, p.s.electric_tariff.export_rates[:WHL])]) / value(m[Symbol("WHL_benefit"*_n)])  <= 1e-3
