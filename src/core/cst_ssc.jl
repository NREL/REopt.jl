...
function run_ssc(case_data::Dict)
    ...
    if !(model in collect(keys(model_ssc)))
        ...
        if model in ["mst","ptc","lf"]
            println("Maximum possible thermal energy collected by CST technology: " * string(round(sum(thermal_production),digits=2)) * " MWht.")
        elseif model in ["swh_evactube","swh_flatplate"]
            println("Maximum possible thermal energy collected by solar water heater: " * string(round(sum(thermal_production),digits=2)) * " kWht.")
        end
