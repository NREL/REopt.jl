...
!!! note "Outage modeling"
    ...
    # Can only use singular or multiple outage modeling inputs, not both
    ...
    With multiple outage modeling, the model will choose to meet the critical loads only if the expected outage cost (see `Financial | value_of_lost_load_per_kwh`) 
    is greater than the microgrid upgrade cost (see `Financial | microgrid_upgrade_cost_fraction`). If you wish to ensure that REopt recommends a system that can meet 
    critical loads during a defined outage period, specify this duration under `Site | min_resil_time_steps`

    # Multiple outage costs will be included in NPV and LCC
