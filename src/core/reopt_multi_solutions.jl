"""
    run_reopt_multi_solutions(fp::String, size_scale::Union{Vector{Any},Vector{Float64}}, ms::AbstractVector{T}; 
                                parallel::Bool=false)  where T <: JuMP.AbstractModel
...
function run_reopt_multi_solutions(fp::String, size_scale::Union{Vector{Any},Vector{Float64}}, ms::AbstractVector{T}; 
    ...
    # TODO add all technologies in techs_possible, but also need to handle different units for size or other things
    techs_possible = ["PV", "Wind", "ElectricStorage", "Generator"]
    techs_evaluated = intersect(techs_possible, keys(input_data))
    techs_sized = String[]
    for tech in techs_evaluated
        if results_dict[tech]["size_kw"] > 0.0
            append!(techs_sized, [tech])
        end
    end
    # If techs were evaluated but sized to zero (no size), then force them to zero in the multi-solution matrix
    techs_to_zero = setdiff(techs_evaluated, techs_sized)
    n_solutions = length(size_scale) * length(techs_sized)

    # Create the first entry for "optimal" solution in the results dictionary
    results_summary = Dict("optimal" => get_multi_solutions_results_summary(results_dict, p, ms[ms_count-1], techs_evaluated))
    ...
    for (i, p) in enumerate(ps)
        ...
        if typeof(rs_solns[i]) <: Dict
            ...
            results_summary[p[1]] = get_multi_solutions_results_summary(results_dict, p[2], ms[n], techs_evaluated)
...
"""
    get_multi_solutions_results_summary(results::Dict, p::REoptInputs, m::JuMP.AbstractModel, techs_evaluated::Vector{String})

Get the results summary dictionary which is a selected number of outputs for Financial, Emissions, and 
the relevant techs

"""
function get_multi_solutions_results_summary(results::Dict, p::REoptInputs, m::JuMP.AbstractModel, techs_evaluated::Vector{String})
    # results_summary always has Financial and emissions results, and then add techs as needed
    results_summary = Dict(
        "status" => results["status"],
        "Net Present Value" => results["Financial"]["npv"],
        "Simple payback period" => get(results["Financial"], "simple_payback_years", 0.0),
        "Net capital cost" => results["Financial"]["lifecycle_capital_costs"],
        "Internal Rate of Return %" => get(results["Financial"], "internal_rate_of_return", 0.0) * 100.0,
        "Site lifecycle CO2 tonnes" => results["Site"]["lifecycle_emissions_tonnes_CO2"]
    )

    # Add techs if they have been evaluated, even if not sized the optimal case
    for key in techs_evaluated
        if key == "PV"
        ...
        elseif key == "ElectricStorage"
            ...
            results_summary["ElectricStorage"] = 
