...
```julia
    min_kw = 0.0, # Minimum electrolyzer size in kW
    max_kw = 1.0e9, # Maximum electrolyzer size in kW
    installed_cost_per_kw = 1413, # Total installed cost per kW of electrolyzer capacity
    om_cost_per_kw = 66.16, # Fixed O&M cost based on power capacity (\$/kW-installed)
    om_cost_per_kwh = 0.0005, # Variable O&M cost based on production (\$/kWh of production)
    require_compression = true, # If true, a compressor is sized and the hydrogen produced by the electrolyzer is compressed to higher pressures before storage
    efficiency_kwh_per_kg = 55.8, # Efficiency of the electrolyzer in kWh of energy required per kg of hydrogen produced
    macrs_option_years = 7, # MACRS schedule for financial analysis (5 or 7 years). Set to zero to disable
    macrs_bonus_fraction = 0.8, # Fraction of upfront project costs to depreciate in Year 1 in addition to scheduled depreciation
    macrs_itc_reduction = 0.5, # Fraction of the ITC value by which the depreciable basis is reduced
    federal_itc_fraction = 0.3, # Fraction of capital costs that are credited towards federal taxes
    ...
    min_turn_down_fraction = 0.0 # Minimum electrolyzer loading in fraction of capacity (size_kw)
```
"""
