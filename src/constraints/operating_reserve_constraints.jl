...
function add_operating_reserve_constraints(m, p; _n="")
	...
	# 1. Production going to load from techs.providing_oper_res 
    ...
    # 2. Total OR required by techs.requiring_oper_res & Load 
    ...
    # 3. Operating reserve provided - available energy stored in battery, adjusted for discharge efficiency 
    @constraint(m, [b in p.s.storage.types.elec, ts in p.time_steps_without_grid],
        ...
        - m[Symbol("dvDischargeFromStorage"*_n)][b,ts] # already AC
    )
    @constraint(m, [b in p.s.storage.types.elec, ts in p.time_steps_without_grid],
        m[Symbol("dvOpResFromBatt"*_n)][b,ts] <= 
    ...
    # 4. Operating reserve provided by techs.providing_oper_res - available capacity after production to load
