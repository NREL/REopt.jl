...
function add_existing_hydropower_constraints(m,p)
	...
	@constraint(m, [ts in p.time_steps, t in p.techs.existing_hydropower],
		...
		m[:dvRatedProduction][t,ts] == m[:dvWaterOutFlow][t,ts] * (p.s.existing_hydropower.efficiency_kwh_per_cubicmeter * p.hours_per_time_step) # convert to kW/time step, for instance: m3/15min  * kwh/m3 * (0.25 hrs/1hr)
	...
	@constraint(m, [ts in p.time_steps[2:end]], m[:dvWaterVolume][ts] == m[:dvWaterVolume][ts-1] + p.s.existing_hydropower.water_inflow_cubic_meter_per_second[ts] - sum(m[:dvWaterOutFlow][t,ts] for t in p.techs.existing_hydropower)) # m[:dvWaterOutFlow][ts]) 
