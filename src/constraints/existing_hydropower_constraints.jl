...
function add_existing_hydropower_constraints(m,p)
	...
	@constraint(m, [ts in p.time_steps],
		p.s.existing_hydropower.minimum_water_output_cubic_meter_per_second <= m[:dvWaterOutFlow][ts]
	)
	@constraint(m, [ts in p.time_steps],
	...
	@constraint(m, [ts in p.time_steps[2:end]], m[:dvWaterVolume][ts] == m[:dvWaterVolume][ts-1] + p.s.existing_hydropower.water_inflow_cubic_meter_per_second[ts] - m[:dvWaterOutFlow][ts])
	@constraint(m, m[:dvWaterVolume][1] == 10) # TODO, set the initial water volume based on a user input
