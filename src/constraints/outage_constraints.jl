...
function add_MG_storage_dispatch_constraints(m,p)
    ...
    @constraint(m, [s in p.s.electric_utility.scenarios, tz in p.s.electric_utility.outage_start_time_steps, ts in p.s.electric_utility.outage_time_steps],
        m[:dvMGStoredEnergy][s, tz, ts] == m[:dvMGStoredEnergy][s, tz, ts-1] + p.hours_per_time_step * (
            ...
            + p.s.storage.attr["ElectricStorage"].dc_charge_efficiency * sum(m[:dvMGProductionToStorage][t, s, tz, ts] for t in p.techs.dc_couple_with_stor)
    ...
    if "ElectricStorage" in p.s.storage.types.dc_coupled
        @info "ElectricStorage in p.s.storage.types.dc_coupled (outages)"
        @constraint(m, [s in p.s.electric_utility.scenarios, tz in p.s.electric_utility.outage_start_time_steps, ts in p.s.electric_utility.outage_time_steps],
        ...
        @constraint(m, [s in p.s.electric_utility.scenarios, tz in p.s.electric_utility.outage_start_time_steps, ts in p.s.electric_utility.outage_time_steps],
            m[:dvStoragePower]["ElectricStorage"] >= m[:dvMGDischargeFromStorage][s, tz, ts]
                ...
                + sum(m[:dvMGProductionToStorage][t, s, tz, ts] for t in p.techs.dc_couple_with_stor) * p.s.storage.attr["ElectricStorage"].inverter_efficiency_fraction #TODO: remove term or leave for future flexibility?
        )
    end
...
function add_MG_dc_coupled_tech_elec_storage_constraints(m, p)
    ...
    @constraint(m, [s in p.s.electric_utility.scenarios, tz in p.s.electric_utility.outage_start_time_steps, ts in p.s.electric_utility.outage_time_steps],
        ...
        m[:dvMGDischargeFromStorage][s, tz, ts] / p.s.storage.attr[b].inverter_efficiency_fraction + 
    ...
    @constraint(m, [s in p.s.electric_utility.scenarios, tz in p.s.electric_utility.outage_start_time_steps, ts in p.s.electric_utility.outage_time_steps],
        m[:dvDCCoupledTechStorageInverterSizeDC]["ElectricStorage"] >= 
        sum(m[:dvMGProductionToStorage][t, s, tz, ts] for t in p.techs.ac_couple_with_stor) * p.s.storage.attr[b].rectifier_efficiency_fraction
    )
