...
function add_storage_size_constraints(m, p, b; _n="")
	...
	@constraint(m,
        m[Symbol("dvStorageEnergy"*_n)][b] <= m[Symbol("dvStoragePower"*_n)][b] * 100 #p.s.storage.attr[b].max_duration_hours
    )

    @constraint(m,
        m[Symbol("dvStorageEnergy"*_n)][b] >= m[Symbol("dvStoragePower"*_n)][b] * 0 #p.s.storage.attr[b].min_duration_hours
    )
