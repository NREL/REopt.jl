...
function add_simultaneous_export_import_constraint(m, p; _n="")
    if solver_is_compatible_with_indicator_constraints(p.s.settings.solver_name)
        ...
        bigM_hourly_load_plus_battery_power = maximum(p.s.electric_load.loads_kw)+maximum(p.s.space_heating_load.loads_kw)+maximum(p.s.process_heat_load.loads_kw)+maximum(p.s.dhw_load.loads_kw)+maximum(p.s.cooling_load.loads_kw_thermal)+sum(p.s.storage.attr[b].max_kw for b in p.s.storage.types.elec)
        @constraint(m, NoGridPurchasesBinary[ts in p.time_steps],
            sum(m[Symbol("dvGridPurchase"*_n)][ts, tier] for tier in 1:p.s.electric_tariff.n_energy_tiers) +
